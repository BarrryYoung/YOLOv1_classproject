...
class VOCDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            # 各种变换用torch自带的的transform做不到，所以通过cv变换
            img, boxes, labels = self.cvtransform(img, boxes, labels)
        h, w, _ = img.shape
    ...
    def random_bright(self, im, delta=16):
        ...
        if alpha > 0.3:
            ...
            im = im.clip(min=0, max=255).astype(int)
        return im



'''
定义Yolodadta类，其中重要的属性有：

self.train_loader:训练集的dataloader
self.test_loader:测试集的dataloader

'''
class Yolodata():
    def __init__(self, train_file_root='./data/VOCdevkit/VOC2007/JPEGImages/',
                 train_listano='voc2007.txt', test_file_root='./data/VOCdevkit/VOC2007/JPEGImages/',
                 test_listano='voc2007.txt', batchsize=2, snumber=7, bnumber=2, cnumber=20):
        self.S = snumber
        self.B = bnumber
        self.C = cnumber
        ...
        img_data = VOCDataset(root=train_file_root, list_file=train_listano, train=True, transform=transform_train,
                              loader=cv_loader, snumber=self.S, bnumber=self.B, cnumber=self.C)
        ...
        img_data_t = VOCDataset(root=test_file_root, list_file=test_listano, train=False, transform=transform_test,
                                loader=cv_loader)
        test_loader = torch.utils.data.DataLoader(img_data_t, batch_size=batchsize, shuffle=False)
    ...
    Yolodatda.test()方法:
    输出测试，训练批次数量
    输出self.train_loader中每个批次的数据和标签大小
    输出self.test_loader中每个批次的数据和标签大小
    就是一个用来看俩dataloader写没写错的方法
    '''
    def test(self):
        # print(len(self.img_data))
        print('there are total %s batches in training and total %s batches for test' % (
...
if __name__ == '__main__':
    testdata = Yolodata(train_file_root='./data/VOCdevkit/VOC2007/JPEGImages/',
                        train_listano='voc2007.txt',
                        test_file_root='./data/VOCdevkit/VOC2007/JPEGImages/',
                        test_listano='voc2007.txt', batchsize=2)
    m, n = testdata.getdata()
    # for img, target in n:
    #     #print("xxx")
    #     pass
    testdata.test()
